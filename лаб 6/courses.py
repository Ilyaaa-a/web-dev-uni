...
def reviews(course_id):
    ...
    if sort_order == 'positive':
        order_clause = Review.rating.desc()
    elif sort_order == 'negative':
        order_clause = Review.rating.asc()
    else:
        order_clause = Review.created_at.desc()
    
    query = db.select(Review).where(Review.course_id == course_id).order_by(order_clause)
